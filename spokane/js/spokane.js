...
var clickedHex = L.geoJson();
var clickedHexArray = [];

var odHexes = L.geoJson();
var odHexesArray = [];
...
function odHexStyle(feature) {
    var colorList = ["#084C8D", "#155593", "#225E99", "#2F689F", "#3C71A5", "#497BAB", "#5684B1", "#638DB7", "#7097BD", "#7DA0C3",
                     "#8AAAC9", "#97B3CF", "#A4BDD5", "#B1C6DB", "#BECFE1", "#CBD9E7", "#D8E2ED", "#E5ECF3", "#F2F5F9"];

    var totalCount = parseInt(feature.properties.count);
    var selectedIndex = 0
    ...
    return {
        fillColor: colorList[selectedIndex],
        fillOpacity: 0.5,
        color: '#fff',
        opacity: 0.75
    }
...
function createSelectedHexPopup(feature, layer) {
    var popup = L.popup().setContent("Total trips: " + feature.properties.count);
    layer.bindPopup(popup);

    var currentStyle = odHexStyle(feature);
...
function createPopup(feature, layer) {
    ...
    layer.on('click', function (e) {
        ...
        var currentHexId = feature.properties.gid;

        // if ($.inArray(currentHexId, Object.keys(od_dict)) > -1) {
        var selectedHexes = od_dict[currentHexId];

        var selectedKeys = Object.keys(selectedHexes);

        for (var i = 0; i < selectedKeys.length; i++){
            var currentFeature = {
                "type":"Feature",
                "properties": {"count": selectedHexes[selectedKeys[i]]},
                "geometry":{
                    "type":"MultiPolygon",
                    "coordinates": shape_dict[parseInt(selectedKeys[i])].geometry.coordinates
                }
            }

            odHexesArray.push(currentFeature);
        }

        odHexes = L.geoJSON(odHexesArray, {
            style: odHexStyle,
            onEachFeature: createSelectedHexPopup
        });

        overlays.addLayer(odHexes).addTo(map);
        overlaysIDs[1] = odHexes._leaflet_id;

        clickedHex = L.geoJSON(clickedHexArray, {
            style: clickedHexStyle
        });

        overlays.addLayer(clickedHex).addTo(map);
        overlaysIDs[2] = clickedHex._leaflet_id;
        // }
    });

    layer.on('mouseover', function(e) {
        this.setStyle({color: '#000', opacity: 0.5})
    });

    layer.on('mouseout', function(e) {
        this.setStyle(hexStyle());
    });
}
...
var od_array = [];
function loadOriginDestination(){
    var jsonFileName = 'json/trip_od.json';
    ...
    $.ajax({
        ...
        success: function(data) {
            for (var i = 0; i < data.features.length; i++) {
                ...
                shape_dict[parseInt(feature.properties.gid)] = feature;
            }
        }
    });
